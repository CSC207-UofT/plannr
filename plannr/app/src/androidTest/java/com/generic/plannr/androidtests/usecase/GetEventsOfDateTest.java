...
public class GetEventsOfDateTest {
    ...
    public void TestAddSchoolEvent() {
        ...
        LocalDate targetTime = LocalDate.of(2021,
                rand.nextInt(11) + 1,
                rand.nextInt(27) + 1);
        ...
        for (int i = 0; i < iteration; i++) {
            LocalDateTime time = LocalDateTime.of(2021,
                    ...
                    0,
                    0);
            if (time.toLocalDate().equals(targetTime)) {
                counter += 1;
            }
            addSchoolEvent(
        ...
        for (SchoolEvent e : EG.getAllEvents(UG.getLoggedInUserID())) {
            eventList.add(e);
        }

        assertEquals(counter, getEventsOfDate(eventList, targetTime).size());
    }
}